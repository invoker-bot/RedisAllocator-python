...
>
> Docker 仍然可以用于运行 Redis、应用 worker 或集成压测。RedisAllocator 不提供 Docker 集群编排、容器调度或原生 Redis Cluster 支持；如果需要分配 Docker 主机或容器槽位，请在业务代码中把它们建模为普通资源 key。
...
*   **范围之外 (当前方向):**
    *   原生 Redis 集群支持。
    *   超出单实例 Lua 能力的多键原子操作。
    *   原生 Docker 集群编排或容器调度。Docker worker 可以使用 RedisAllocator，但应将主机、端口、GPU 或容器槽位作为普通资源 key 管理。
