...
# Run the default fast unit suite (excludes `concurrency`, `fuzz`, `stress`, and `benchmark` markers)
pytest
...
- **`assign()` / `shrink()` / `clear()` are not namespace scrubbers.** `assign()` and `shrink()` remove pool membership but intentionally leave lock keys for removed members; `clear()` deletes the pool hash/head/tail only. This is deliberate to avoid write amplification and wildcard scans. If an evicted permanent lock becomes an orphan, recover it with `free_keys(key)`, by reintroducing that same key through `extend([key])` or `assign([... key ...])`, or by deleting `<prefix>|<suffix>:<key>` directly.
